...
use crate::storage::{
    ...
    get_verified_count as storage_get_verified_count, has_record, remove_from_index, remove_record,
    require_initialized, set_count, set_record, set_verified_count, ADMIN_KEY,
};
...
mod test {
    ...
    #[test]
    fn test_partial_removal_stats_preserve_verified_remaining_record_wave_46() {
        let env = Env::default();
        let (_admin, user1, user2, contract_id) = setup(&env);
    ...
    fn test_cold_start_register_exposes_dashboard_state_wave_50() {
        ...
        env.as_contract(&contract_id, || {
            ...
            let all = TrustBridgeContract::get_all_registered(env.clone()).unwrap();
            assert_eq!(all.len(), 1);
            assert_eq!(all.get(0).unwrap(), (username(&env, "octocat"), user));
        });
    }
